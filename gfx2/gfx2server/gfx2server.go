...
func Grafikzeilen() uint16 {
	if !FensterOffen() {
		panic("Das gfx-Fenster ist nicht offen!")
	}
	return fensterhoehe
}
...
func starteGfxServer(quellIP string, portnummer uint16, f func(string) string) {
	...
	if err != nil {
		panic("Eine Verbindung ist fehlgeschlagen!")
	}
	if serverprotokoll {
		fmt.Println("Verbunden mit:", conn.RemoteAddr())
	}
	connMaus, err2 := server.Accept() // Mausanfragen
	if err2 != nil {
		panic("Verbindung für Mausanfragen ist fehlgeschlagen!")
	}
	if serverprotokoll {
		fmt.Println("Maus-Verbindung über:", conn.RemoteAddr())
	}
	connTast, err3 := server.Accept() // Tastaturanfragen
	if err3 != nil {
		panic("Verbindung für Tastaturanfragen ist fehlgeschlagen!")
	}
	if serverprotokoll {
		fmt.Println("Tastatur-Verbunden über:", conn.RemoteAddr())
	}
	for !FensterOffen() {
		time.Sleep(1e8)
	} //Solange das Fenster noch nicht offen ist, warte ...
	go func() {
		for FensterOffen() {
